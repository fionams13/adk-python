...
from google.adk.tools.bigtable.bigtable_credentials import BIGTABLE_TOKEN_CACHE_KEY
from google.adk.tools.bigtable.bigtable_credentials import BigtableCredentialsConfig
from google.adk.tools.bigtable.bigtable_credentials import BigtableCredentialsManager
...
class TestBigtableCredentialsManager:
  """Test suite for BigtableCredentialsManager OAuth flow handling.
  ...
  def credentials_config(self):
    ...
    return BigtableCredentialsConfig(
  ...
  def manager(self, credentials_config):
    ...
    return BigtableCredentialsManager(credentials_config)
  ...
  async def test_get_credentials_from_cache_when_none_in_manager(
    ...
    mock_tool_context.state[BIGTABLE_TOKEN_CACHE_KEY] = mock_cached_creds_json
  ...
  async def test_refresh_cached_credentials_success(
    ...
    with patch(
      ...
      assert (
          "new_token"
          == json.loads(mock_tool_context.state[BIGTABLE_TOKEN_CACHE_KEY])[
              "token"
          ]
      )
  ...
  async def test_oauth_flow_completion_with_caching(
    ...
    mock_creds_json = (
        ...
        ' ["https://www.googleapis.com/auth/bigtable.data"], "universe_domain":'
    ...
    with patch(
        "google.adk.tools.bigtable.bigtable_credentials.google.oauth2.credentials.Credentials",
      ...
      assert (
          mock_tool_context.state[BIGTABLE_TOKEN_CACHE_KEY] == mock_creds_json
      )
  ...
  async def test_cache_persistence_across_manager_instances(
    ...
    manager1 = BigtableCredentialsManager(credentials_config)
    ...
    with patch(
      ...
      assert BIGTABLE_TOKEN_CACHE_KEY in mock_tool_context.state
      cached_creds_json = mock_tool_context.state[BIGTABLE_TOKEN_CACHE_KEY]
      assert cached_creds_json == mock_creds_json

    # Create second manager instance (simulating new request/session)
    manager2 = BigtableCredentialsManager(credentials_config)
    ...
    with patch(
        "google.adk.tools.bigtable.bigtable_credentials.google.oauth2.credentials.Credentials.from_authorized_user_info"
    ) as mock_from_json:
      mock_cached_creds = Mock(spec=OAuthCredentials)
      mock_cached_creds.valid = True
      mock_from_json.return_value = mock_cached_creds
