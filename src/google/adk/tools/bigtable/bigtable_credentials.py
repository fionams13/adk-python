...
BIGTABLE_DEFAULT_SCOPE = ["https://www.googleapis.com/auth/bigtable.data"]


@experimental
class BigtableCredentialsConfig(BaseModel):
  ...
  all the required Bigtable resource. Setting this credential to allow user to
  access the Bigtable resource without end users going through oauth flow.
  ...
  Example usage 2: When the agent wants to access the user's Bigtable resources
...
class BigtableCredentialsManager:
  ...
  def __init__(self, credentials_config: BigtableCredentialsConfig):
  ...
  async def get_valid_credentials(
    ...
    creds_json = tool_context.state.get(BIGTABLE_TOKEN_CACHE_KEY, None)
    creds = (
    ...
    if creds and creds.expired and creds.refresh_token:
      try:
        ...
        if creds.valid:
          ...
          tool_context.state[BIGTABLE_TOKEN_CACHE_KEY] = creds.to_json()
          return creds
      except RefreshError:
